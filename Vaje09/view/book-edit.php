...
<form action="<?= BASE_URL . "book/edit" ?>" method="post">
    ...
    <p><label>Author: <input type="text" name="author" value="<?= $book["author"] ?>" autofocus required pattern="^[ a-zA-ZšđčćžŠĐČĆŽ\.\-]+$"/>
    ...
    <p><label>Price: <input type="text" name="price" value="<?= $book["price"] ?>" required min="0" step="any"/>
        <span class="important"><?= $errors["price"] ?></span></label></p>
    <p><label>Year: <input type="text" name="year" value="<?= $book["year"] ?>" required min="1500" max="2020"/>
        <span class="important"><?= $errors["year"] ?></span></label></p>
    <p><label>Quantity: <input type="text" name="quantity" value="<?= $book["quantity"] ?>" required min="10"/>
